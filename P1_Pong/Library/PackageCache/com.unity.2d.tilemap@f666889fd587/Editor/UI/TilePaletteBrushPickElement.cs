...
namespace UnityEditor.Tilemaps
{
    ...
    internal partial class TilePaletteBrushPickElement : VisualElement
    {
        ...
        private static readonly string kEmptyViewSaveBrushPickText2 = L10n.Tr("<indent=1em>\u2022<indent=2em>Pick with the Brush from the Tile Palette or Scene View");
        ...
        private readonly int[] m_CachedSelectionIndex = { 0 };
        ...
        private void UpdateSize()
        {
            ...
            m_PersonalGridView.SetItemHeightAndColumnCount(itemSize, gridColumnCount);
        ...
        private void UpdatePersonalDisplay()
        {
            ...
            if (showGrid)
            {
                ...
                m_CachedSelectionIndex[0] = m_PersonalListView.selectedIndex;
                if (m_CachedSelectionIndex[0] == -1)
                    m_PersonalGridView.SetSelectionWithoutNotify(Array.Empty<int>());
                else
                    m_PersonalGridView.SetSelectionWithoutNotify(m_CachedSelectionIndex);
            ...
            else
            {
                ...
                m_CachedSelectionIndex[0] = m_PersonalGridView.selectedIndex;
                if (m_CachedSelectionIndex[0] == -1)
                    m_PersonalListView.SetSelectionWithoutNotify(Array.Empty<int>() as IEnumerable<int>);
                else
                    m_PersonalListView.SetSelectionWithoutNotify(m_CachedSelectionIndex as IEnumerable<int>);
        ...
        {
            UpdateEmptyPicksView();
            var hideOnPickTemp = hideOnPick;
            hideOnPick = false;
            UpdatePick();
            hideOnPick = hideOnPickTemp;
        }
        ...
        private void ScrollToSelected(int index)
        {
            ...
            m_CachedSelectionIndex[0] = index;
            if (index < m_PersonalListView.itemsSource.Count)
                m_PersonalListView.SetSelectionWithoutNotify(m_CachedSelectionIndex as IEnumerable<int>);
            if (index < m_PersonalGridView.itemsSource.Count)
                m_PersonalGridView.SetSelectionWithoutNotify(m_CachedSelectionIndex);
