...
namespace UnityEditor.Tilemaps
{
    internal abstract class PaintableGrid : ScriptableObject
    {
        ...
        protected void OnMouseUpOutside()
        {
            if (CanPickOutsideEditMode() || inEditMode)
            {
                HandleBrushPicking();
        ...
        private void HandleMoveTool()
        {
            ...
            if (evt.rawType == EventType.MouseUp && evt.button == 0 && m_PreviousMove.HasValue && TilemapEditorTool.IsActive(typeof(MoveTool)) && isHotControl)
            {
                m_PreviousMove = null;
        ...
        private void HandleBoxTool()
        {
            ...
            if (evt.rawType == EventType.MouseUp && evt.button == 0 && TilemapEditorTool.IsActive(typeof(BoxTool)))
            {
                if (IsMouseUpInWindow() && isHotControl && m_MarqueeStart.HasValue)
                {
                    RegisterUndo();
                    RectInt rect = GridEditorUtility.GetMarqueeRect(m_MarqueeStart.Value, mouseGridPosition);
                    if (evt.shift)
