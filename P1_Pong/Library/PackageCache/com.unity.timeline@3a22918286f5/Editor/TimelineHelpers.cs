...
namespace UnityEditor.Timeline
{
    static class TimelineHelpers
    {
        ...
        public static string GetTrackCategoryName(System.Type trackType)
        {
            ...
            // if a display name with a path is specified use that
            ...
            {
                var name = attr.DisplayName.TrimStart('/');
                var lastSlash = name.LastIndexOf('/');
                return lastSlash >= 0 ? name.Substring(0, lastSlash + 1) : string.Empty;
            }
        ...
        {
            var name = TypeUtility.GetDisplayName(trackType).TrimStart('/');
            var lastSlash = name.LastIndexOf('/');
