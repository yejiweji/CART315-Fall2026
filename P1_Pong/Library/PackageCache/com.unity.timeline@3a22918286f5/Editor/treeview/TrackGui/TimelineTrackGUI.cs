...
namespace UnityEditor.Timeline
{
    class TimelineTrackGUI : TimelineGroupGUI, IClipCurveEditorOwner, IRowGUI
    {
        ...
        void DrawTrackBinding(Rect rect)
        {
            ...
            // By default the size is just the width of the string (for selection purposes) capped by the entire available width.
            rect.width = Math.Min(m_Styles.trackHeaderFont.CalcSize(new GUIContent(trackName)).x, availableWidth);


#if UNITY_6000_0_OR_NEWER
            textStyle.clipping = TextClipping.Ellipsis;
            // If we are editing, supply the entire available width of the header and remove ellipsis for long names.
...
#else
            textStyle.clipping = TextClipping.Clip;
            // If we are editing, supply the entire available width of the header
            if (GUIUtility.keyboardControl == track.GetObjectId().GetHashCode())
                rect.width = availableWidth;
#endif
            trackName = EditorGUI.DelayedTextField(rect, GUIContent.none, track.GetObjectId().GetHashCode(),
                track.name, textStyle);
