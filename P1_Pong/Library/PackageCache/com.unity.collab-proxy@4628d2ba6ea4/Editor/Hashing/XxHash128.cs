...
// - Inherits from the ported NonCryptographicHashAlgorithm base class.
// - UInt128-based members (#if NET) removed.
// - System.Runtime.CompilerServices.Unsafe and System.Numerics.BitOperations
//   usages replaced to avoid extra assembly dependencies.
// Produces the same digests, in the same (big-endian) byte order, as
// System.IO.Hashing.XxHash128 - the implementation used by the rest of
// the Plastic SCM clients.
...
using static Unity.PlasticSCM.Editor.Hashing.XxHashShared;
...
namespace Unity.PlasticSCM.Editor.Hashing
{
    ...
    internal sealed unsafe class XxHash128 : NonCryptographicHashAlgorithm
    {
        ...
        /// <summary>Gets the current computed 128-bit hash value without modifying accumulated state.</summary>
        /// <returns>The hash value for the data already provided, in Big Endian byte order.</returns>
        public new byte[] GetCurrentHash()
        {
            byte[] result = new byte[HashLengthInBytes];
            Hash128 current = GetCurrentHashAsHash128();
            WriteBigEndian128(current, result);
            return result;
        }
        ...
        private new static void ThrowDestinationTooShort() =>
        ...
        private static Hash128 HashLength1To3(byte* source, uint length, ulong seed)
        {
            ...
            uint combinedh = RotateLeft(BinaryPrimitives.ReverseEndianness(combinedl), 13);
