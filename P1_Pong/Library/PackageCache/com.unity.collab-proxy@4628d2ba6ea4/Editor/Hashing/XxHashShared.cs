...
// - SIMD intrinsics paths (#if NET) removed; the scalar fallbacks are kept.
...
{
    /// <summary>Shared implementation of the XXH3 hash algorithm for the <see cref="XxHash128"/> version.</summary>
    internal static unsafe class XxHashShared
    {
        ...
        // Replacements for System.Numerics.BitOperations.RotateLeft, which is not
        // available in the Unity editor's netstandard2.1 profile.
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        // Plain pointer dereferences instead of Unsafe.ReadUnaligned/WriteUnaligned:
        // every platform the Unity editor runs on (x64/arm64) supports unaligned
        // scalar loads and stores, and this avoids a dependency on the
        // System.Runtime.CompilerServices.Unsafe assembly.
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static uint ReadUInt32LE(byte* data) =>
            BitConverter.IsLittleEndian ?
                *(uint*)data :
                BinaryPrimitives.ReverseEndianness(*(uint*)data);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ulong ReadUInt64LE(byte* data) =>
            BitConverter.IsLittleEndian ?
                *(ulong*)data :
                BinaryPrimitives.ReverseEndianness(*(ulong*)data);
