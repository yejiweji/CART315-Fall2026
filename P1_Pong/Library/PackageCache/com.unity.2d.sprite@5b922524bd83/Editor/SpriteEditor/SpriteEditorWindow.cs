...
namespace UnityEditor.U2D.Sprites
{
    ...
    internal class SpriteEditorWindow : SpriteUtilityWindow, ISpriteEditor, ISupportsOverlays
    {
        ...
        SpriterEditorHintPopup m_HintPopup;
        ...
        internal UnityEngine.Object selectedObject
        {
            ...
            set
            {
                if(m_ScenePreviewToggle != null)
                    m_ScenePreviewToggle.value = false;
        ...
        private bool SetupVisualElements()
        {
            ...
            if (styleSheet != null && rootVisualElement != null && rootVisualElement.styleSheetList != null)
            {
                ...
                m_HintPopup = new SpriterEditorHintPopup();
                m_HintPopup.icon = EditorGUIUtility.GetHelpIcon(MessageType.Warning);
                m_HintPopup.hintText = SpriteEditorWindowStyles.editingDisableMessageBecausePlaymodeLabel.text;
                m_HintPopup.style.display = DisplayStyle.None;
                baseRootVisualElement.Add(m_HintPopup);
                return true;
            }
            return false;
        }
        ...
        {
            if(m_ScenePreviewToggle != null)
                m_ScenePreviewToggle.SetEnabled(playModeState == PlayModeStateChange.EnteredEditMode);
            if (PlayModeStateChange.ExitingEditMode == playModeState || PlayModeStateChange.ExitingPlayMode == playModeState)
            {
                RebuildCache();
            }
        }
        ...
        void ShowEditorMessage(string message, MessageType messageType)
        {
            ...
            {
                if(m_HintPopup.style.display == DisplayStyle.Flex)
                    m_HintPopup.style.display = DisplayStyle.None;
            }
            else if(m_HintPopup.hintText != message || m_HintPopup.style.display != DisplayStyle.Flex)
            {
                m_HintPopup.hintText = message;
                m_HintPopup.icon = EditorGUIUtility.GetHelpIcon(messageType);
                m_HintPopup.style.display = DisplayStyle.Flex;
            }
        }
        ...
        private void DoTextureAndModulesGUI()
        {
            ...
            ShowEditorMessage(null, MessageType.None);
            textureViewRect = new Rect(0f, 0f, m_MainViewIMGUIElement.layout.width - k_ScrollbarMargin, m_MainViewIMGUIElement.layout.height - k_ScrollbarMargin);
